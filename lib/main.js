...
function flattenClbk( value, indices ) {
	if ( !isNonNegativeInteger( value ) ) {
		throw new TypeError( format( '1H1Gv', indices.join( ', ' ), String( value ) ) );
	}
	return value;
}
...
function copy2buffer( x, shape ) {
	...
	for ( i = 0; i < N; i++ ) {
		...
		if ( !isNonNegativeInteger( v ) ) {
			throw new TypeError( format( '1H1Gv', ind2sub( shape, sx, ox, ord, i, MODE ), String( v ) ) );
		}
		out[ i ] = v;
	}
	return out;
...
function chi2test( x, options ) {
	...
	if ( isndarrayLike( x ) ) {
		sh = x.shape;
		if ( sh.length !== 2 ) {
			throw new TypeError( format( '1H1Gw', sh.length ) );
		}
		...
		throw new TypeError( format( '1H1Gx', x ) );
	}
